...
async function getAllUsers() {
  const [rows] = await pool.query(
    'SELECT name, emp_no AS empNo, phone, dept_name AS deptName, position, cost_type AS costType, role FROM users ORDER BY id ASC'
  );
  return rows;
}
...
async function createUser(name, empNo, phone, role, password, mustChange, deptName, position, costType) {
  if (mustChange === undefined) mustChange = true;
  var hash = await makePassword(password);
  return pool.query(
    'INSERT INTO users (name, emp_no, phone, role, password, must_change_password, dept_name, position, cost_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [name, empNo, phone, role, hash, mustChange ? 1 : 0, deptName || null, position || null, costType || '운영']
  );
}

async function updateUser(empNo, name, phone, role, deptName, position, costType) {
  var parts = [];
  var vals = [];
  ...
  if (deptName !== undefined) { parts.push('dept_name = ?'); vals.push(deptName || null); }
  if (position !== undefined) { parts.push('position = ?'); vals.push(position || null); }
  if (costType)  { parts.push('cost_type = ?'); vals.push(costType); }
